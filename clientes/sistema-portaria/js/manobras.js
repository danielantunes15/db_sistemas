...
document.getElementById('form-manobra').addEventListener('submit', function(e) {
    ...
    const localManobra = formData.get('local_manobra');
    
    const manobra = {
        id: Date.now(),
        placa_veiculo: formData.get('placa_veiculo').toUpperCase(),
        motorista: formData.get('motorista'),
        local_manobra: localManobra, // (ALTERADO) Agora é um texto (ex: "Nanuque-MG")
        horario_previsto: horarioPrevisto.toLocaleString('pt-BR'),
        horario_timestamp: horarioPrevisto.getTime(),
        
        // (NOVO) Salva as coordenadas do destino para o mapa
        destino_coords: getCoordsFromLocationName(localManobra), 
...
function iniciarManobra(id) {
    ...
    if (manobra) {
        ...
        manobra.horario_inicio = agora.toLocaleString('pt-BR');
        // (NOVO) Salva o timestamp de início para o cálculo da simulação
        manobra.horario_inicio_timestamp = agora.getTime(); 
        salvarDados();
        carregarManobras();
    }
}
